...
import oceannet.com.ziparisyonetim.UTILS.Adapters.Adapter_siparisGecmis;
import oceannet.com.ziparisyonetim.UTILS.AndyConstants;
import oceannet.com.ziparisyonetim.UTILS.RecyclerItemClickListener;

public class Avt_gecmisSiparisler extends AppCompatActivity
    ...
    Adapter_siparisGecmis adapter_siparisler,adapter_siparisler_Accept;
    ArrayList<OrderPast> orderGelen = new ArrayList<OrderPast>();
    ArrayList<OrderCurrent> orderAccept = new ArrayList<OrderCurrent>();

    OrderPast currentOrder;
    DrawerLayout drawer;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_avt_gecmis_siparisler);
        ...
        setDialog();
        viewAlert1.setBackground(ActivityCompat.getDrawable(this,R.drawable.bcg_item_w));
        viewAlert2.setBackground(ActivityCompat.getDrawable(this,R.drawable.bcg_item_w));
...
//
    ...
    public void getGelen(){
        ...
        pDialog.show();
        AndroidNetworking.get(AndyConstants.URL_GETUSERPASTORDER)
                            ...
                            if (status == 200){
                                ...
                                for (int i = 0; i <data.length() ; i++) {
                                    ...
                                    for (int j = 0; j <jsoorder.getJSONArray("OrderListDetails").length() ; j++) {
                                     ...
                                     int rate = -1;
                                    if (jsoorder.get("OrderRate") != JSONObject.NULL) {
                                        rate = jsoorder.getInt("OrderRate");
                                    }
                                    OrderPast order = new OrderPast(jsoorder.getInt("OrderID"),jsoorder.getInt("Status"),jsoorder.getString("RoomName"),jsoorder.getString("FullName"),jsoorder.getString("Email"),jsoorder.getString("Description"),jsoorder.getString("BlockName"),jsoorder.getString("FloorName"),jsoorder.getString("CompanyName"),jsoorder.getString("OrderDateTime"),jsoorder.getString("OrderDeliverTime"),jsoorder.getString("OrderRateText"),rate,products);
    ...
    public void initializeGelen(){
        ...
        adapter_siparisler = new Adapter_siparisGecmis(this,this,orderGelen);
        rcl_gelen.setLayoutManager(manager2);
        rcl_gelen.setAdapter(adapter_siparisler);



        rcl_gelen.addOnItemTouchListener(new RecyclerItemClickListener(this, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
    ...
    public void initializeAlert(){
        ...
        btn_teslimEt.setVisibility(View.GONE);

        txt_status.setVisibility(View.VISIBLE);

        txt_status.setText("Teslim Edildi");


        txt_comments.setText(currentOrder.getDescription());
        if (currentOrder.getOrderRate() > 0){



            ratingBar.setVisibility(View.VISIBLE);
            ratingBar.setRating(currentOrder.getOrderRate());
            txt_ratingStatus.setVisibility(View.GONE);

        }   else{

            txt_ratingStatus.setVisibility(View.VISIBLE);
            txt_ratingStatus.setText("Değerlendirme Bekliyor");
        ...
        Adapter_product adapter_siparisler = new Adapter_product(this,currentOrder.getProducts());
